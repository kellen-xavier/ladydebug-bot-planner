...
def test_start_day_cria_sessao_aberta(storage, clock):
...
def test_nao_permite_dois_dias_abertos(storage, clock):
    svc = DayService(storage, clock)
    svc.start_day("u1", "c1")
...
def test_add_entry_anexa_ao_dia_aberto(storage, clock):
    svc = DayService(storage, clock)
    svc.start_day("u1", "c1")
...
def test_add_entry_sem_dia_aberto_falha(storage, clock):
    svc = DayService(storage, clock)
    with pytest.raises(NoOpenDay):
        svc.add_entry("u1", Entry(type=EntryType.NOTA, raw_input="x"))


def test_close_day_fecha_e_soma_voz(storage, clock):
    svc = DayService(storage, clock)
    svc.start_day("u1", "c1")
    svc.voice_join("u1")
...
def test_close_day_fecha_intervalo_de_voz_pendente(storage, clock):
    svc = DayService(storage, clock)
    svc.start_day("u1", "c1")
    svc.voice_join("u1")
